...
func main() {
	...
	seatRepo := repository.GetSeatRepository(db)
	eventSvc := service.GetEventService(eventRepo, seatRepo, lockFactory)
	bookingSvc := service.GetBookingService(seatRepo, lockFactory)
	bookingCtrl := controller.GetBookingController(eventSvc, bookingSvc)
	...
	bookingCtrl.RegisterRoutes(router)
